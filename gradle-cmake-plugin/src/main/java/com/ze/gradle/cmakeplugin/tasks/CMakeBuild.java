...
public class CMakeBuild extends DefaultTask {
    ...
    private final RegularFileProperty binary = getProject().getObjects().fileProperty().convention(getProject().getLayout().getProjectDirectory().file("lib"));
    ...
    public void executeMake() {
        ...
        getProject().exec(execSpec -> {
            ...
            execSpec.commandLine(
                    cmakeExecutable,
                    "--build",
                    getVariantDirectory().get().getAsFile().getAbsolutePath());
        });
    }
